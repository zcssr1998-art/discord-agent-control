...
- [x] `npm run check` 鈥?102 files, 0 failed
...
> npm run check   鈫?checked 102 file(s), 0 failed
...
Deterministic: `tests/v4-p22-insert.test.mjs` (10 tests) + updated P2.1/P2.2 suites (`npm test 302/0).
...
local equivalents: npm test 302/0, npm run check 102/0
```
...
```text
tests 302 pass / 0 fail after extraction + ACK hardening + live-insert steering (baseline at branch point: 244 pass / 0 fail)
...
tests: 302/0 · check 102/0 · smoke:p2 11/11 · smoke:p22 10/10 · smoke:p22-insert 14/14 · smoke:p22-model 5/5
